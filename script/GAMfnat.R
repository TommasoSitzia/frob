...
Data$nonzerofnat=ifelse(Data$fnat == 0, 0, 1)
Data$nonzerofnat=as.integer(Data$nonzerofnat)

names(Data)

##########################################
Binomial_GAM_W_nat  <- gam(nonzerofnat ~ s(bio1,bs="cr") +  s(bio12,bs="cr")  +  
                       s(tconmean,bs="cr") + s(lenferr,bs="cr") +  
                       s(lenroadr,bs="cr")  +  s(shdi,bs="cr")  + s(popde,bs="cr")+s(urbr,bs="cr")+s(popdi,bs="cr")+s(numazr,bs="cr")+s(redre,bs="cr")+s(coorx,coory, bs = "gp", m = 2),weights=areagis_ha/mean(areagis_ha),                    
                     data=Data,family= binomial(link="logit"),method="REML")

summary(Binomial_GAM_W_nat)

# 1. Calculate Predicted Probabilities
predicted_probs <- predict(Binomial_GAM_W_nat, type = "response")  # Replace 'Model' with your GAM model

# 2. Calculate AUC
roc_obj <- roc(Data$nonzerofnat, predicted_probs)  # Replace 'Data' with your dataset
...
Preds_nzero_specific <- predict(Binomial_GAM_W_nat, newdata = new_data, type = "link", se = TRUE, unconditional = TRUE)
...
for (i in 1:num_repetitions) {
  ...
  M1 <- gam(nonzerofnat ~ s(bio1,bs="cr") +  s(bio12,bs="cr")  +  
              s(tconmean,bs="cr") + s(lenferr,bs="cr") +  
  ...
  roc_obj <- roc(test_data$nonzerofnat, predicted_probs)
...
# Assuming Binomial_GAM_W_nat is your GAM model
model_summary <- summary(Binomial_GAM_W_nat)
...
all_vars <- setdiff(names(Data), "nonzerofnat")
...
for (var in significant_vars) {
  ...
  Preds_nzero <- predict(Binomial_GAM_W_nat, newdata = R, type = "link", se = T, unconditional = T) 
  ...
  gg_original[[var]] <- data.frame(x = Data[I, var], y = Data[I, "nonzerofnat"], var = rep(var, sum(I)))
}
...
# Plot via ggplot2
# Further decrease the size of the overall graph
pdf("Binomial_GAM_nat.pdf", width = 6, height = 6) # Fig. 5
...
ggplot(ggg_x, aes(x = x, y = Predictions, group = var)) +
  ...
  ylab(expression(paste("Probability of ", ~f[nat] > 0, sep = ""))) +
  xlab("") +
...
#######################################################################################
## The Second Stage
### 2) Model  fnat when fnat > 0 throug Beta_GAM with logistic link
###
r <- 35
Beta_GAM_W_nat    <- gam(fnat ~ s(bio1,bs="cr",k=r) +  s(bio12,bs="cr",k=r)  +  s(tconmean,bs="cr",k=r) + 
                           s(lenferr,bs="cr",k=r) +
                         ...
                         subset=fnat>0,data=Data,family=betar(link="logit"),method="REML")

summary(Beta_GAM_W_nat)
...
model_summary <- summary(Beta_GAM_W_nat)
...
# Create a baseline data frame with median values for all predictors
# Including 'coorx' and 'coory' as they are required for the model prediction
all_vars <- setdiff(names(Data), "fnat")
baseline_data <- as.data.frame(lapply(Data[, all_vars], median))
...
for (var in significant_vars) {
  ...
  # Predict and Produce confidence intervals:
  #
  Preds_frob  <- predict(Beta_GAM_W_nat,newdata = R,type="link",se=T,unconditional=T) 
  fit.link    <- Preds_frob$fit 
  se          <- Preds_frob$se 
  CI.L        <- fit.link-2*se 
  CI.R        <- fit.link+2*se 
  CI          <- cbind(fit.link,CI.L,CI.R) 
  CI          <- exp(CI)/(1+exp(CI)) # The first column corresponds to the estimated average of fnat when fnat > 0
  colnames(CI) <- c("Predictions","CI_L","CI_R")
  # Store data for plotting
  gg_x[[var]] <- data.frame(cbind(CI, x = R[, var]), var = rep(var, nn))
  ...
  # Original data for rug plots
  I <- Data[, var] > a & Data[, var] < b
  gg_original[[var]] <- data.frame(x = Data[I, var], y = Data[I, "fnat"], var = rep(var, sum(I)))
}
...
pdf("Beta_GAM_nat.pdf", width = 6, height = 6) # Fig. 7
...
ggplot(ggg_x, aes(x = x, y = Predictions, group = var)) +
  ...
  ylab(expression(paste("Average of ", ~f[nat], " given that ", ~f[nat] > 0, sep = ""))) +
  xlab("") +
...
significant_vars_beta <- extract_significant_vars(summary(Beta_GAM_W_nat))
significant_vars_binomial <- extract_significant_vars(summary(Binomial_GAM_W_nat))
...
for(i in 1:p){
  ...
  #
  fit_Binomial = predict(Binomial_GAM_W_nat,newdata=R,type="response",se=T,unconditional=T) 
  fit_Beta     = predict(Beta_GAM_W_nat,newdata=R,type="response",se=T,unconditional=T)     
  ...
  ## 
  phi <- Beta_GAM_W_nat$family$getTheta(TRUE)
  ...
  gg_original[[i]] <- data.frame(x=Data[I,x[i]],y=Data[I,"fnat"],var = rep(var.names[i],sum(I)))
}        
...
# Plot
pdf("Hurdle_GAM_nat.pdf", width = 6, height = 6)  # Adjust the PDF size # Fig. S2
...
ggplot(ggg_x[ggg_x$var %in% all_significant_vars, ], aes(x = x, y = Predictions, group = var, fill = var)) +
  ...
  ylab(expression(paste(~f[nat], sep = ""))) +
  xlab("") +
  theme(legend.background = element_rect(fill = "white"),
        legend.key = element_rect(fill = "white", color = "white"),
        plot.background = element_rect(fill = "white"),
...
info_M1 <- extract_gam_info(Binomial_GAM_W_nat)
significant_vars_M1 <- filter_significant(info_M1)
info_M2 <- extract_gam_info(Beta_GAM_W_nat)
...
results_M1 <- create_results_df(info_M1, "Binomial_GAM_W_nat", significant_vars)
results_M2 <- create_results_df(info_M2, "Beta_GAM_W_nat", significant_vars)
...
# Print results
print("Results
for Binomial_GAM_W_nat:")
print(results_M1)
print("Results for Beta_GAM_W_nat:")
print(results_M2)
